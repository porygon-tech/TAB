...
```{r answerP1.1}
rd <- read.table('C:/Users/Miki/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu18.04onWindows_79rhkp1fndgsc/LocalState/rootfs/home/mroman/TAB/ProjectP1/mikel-20191231T125357Z-001/mikel/IDD.txt', sep='\t', header=FALSE)
head(rd)
...
head(idd)
ggplot(data = idd, mapping = aes(x = length, y = count)) +
  geom_col() + 
...
```{r answerP1.6}
ggplot(data = pi, mapping = aes(y = PI, x = BIN_START)) +
  geom_line(color ="red4") +
...
ggplot(data = snpd, mapping = aes(x = BIN_START, y = VARIANTS.KB)) +
  geom_line(color ="red4") +
...
ggplot(data = VEP, mapping = aes(x = Consequence)) + 
  geom_bar(width = 1) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
