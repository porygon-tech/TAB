...
`ggplot2` is a data visualization package for the statistical programming language R ([Wickham 2009](https://www.springer.com/gp/book/9780387981413)). It was created by Hadley Wickham, implementing  Leland Wilkinson's Grammar of Graphics â€”a general scheme for data visualization which breaks up graphs into semantic components ([Wilkinson 2010](https://onlinelibrary.wiley.com/doi/full/10.1002/wics.118)).
...
file, which has a header and itâ€™s a tab-delimited file, we will write:
...
<div style = "background-color:#FFDAA1">
  <i class="fas fa-info-circle"></i> The data was extracted from the 1974 Motor Trend US magazine, and comprises fuel consumption and 10 aspects of automobile design and performance for 32 automobiles (1973â€“74 models). You can type `??mtcars` in the `R` console to read a description of the data.
</div>
...
<div style="background-color:#F0F0F0">
##### <i class="fas fa-comment-dots"></i> Answer:
It fills the whole column, showing only information about the proportions.
</div>
...
ggplot(data = mtcars, mapping = aes(x = cyl_f, fill = gear)) +
  geom_bar(position = "fill")
ggplot(data = mtcars, mapping = aes(x = cyl_f, fill = as.factor(gear))) +
  geom_bar(position = "fill")
...
<div style="background-color:#F0F0F0">
##### <i class="fas fa-comment-dots"></i> Answer:
Los histogramas se muestran adosados horizontalmente:
```{r diamonds-facetsanswer3.1}
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(. ~ cut)
```
...
<div style="background-color:#F0F0F0">
##### <i class="fas fa-comment-dots"></i> Answer:
El apilamiento vertical muestra de forma mas clara las diferencias en la variable de interes, en este caso 'carat'.
</div>
...
Tag permite añadir un titulo o anotacion, mientras que subtitle añade un subtítulo y caption una nota al pie.
```{r answer4.2}
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) +
...
```{r answerP1.1}
rd <- read.table('C:/Users/Miki/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu18.04onWindows_79rhkp1fndgsc/LocalState/rootfs/home/mroman/TAB/ProjectP1/mikel-20191231T125357Z-001/mikel/', sep='\t', header=FALSE)
head(rd)

#C:/Users/Miki/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu18.04onWindows_79rhkp1fndgsc/LocalState/rootfs/home/mroman/TAB/ProjectP1/mikel-20191231T125357Z-001/mikel

ggplot(data = rd, mapping = aes(x = V4, y = V3)) +
  geom_point()
...
st <- read.table('C:/Users/Miki/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu18.04onWindows_79rhkp1fndgsc/LocalState/rootfs/home/mroman/TAB/ProjectP1/mikel-20191231T125357Z-001/mikel/ST.txt', sep='\t', header=FALSE)
...
```{r answerP1.4}
pl1 <- ggplot(data = st, mapping = aes(x = type, y = count)) +
  geom_col() + 
  ggtitle("Substitutions") + 
  scale_x_discrete(name = "Type of substitution") + 
  scale_y_continuous(name = "Total number") + 
  theme_bw()
pl1
ggsave(filename = "subs.png", plot = pl1, width = 6, height = 4)
...
```{r answerP1.5}
idd <- read.table('C:/Users/Miki/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu18.04onWindows_79rhkp1fndgsc/LocalState/rootfs/home/mroman/TAB/ProjectP1/mikel-20191231T125357Z-001/mikel/IDD.txt', sep='\t', header=FALSE)
colnames(idd) <- c("IDD", "ID", "length", "count")
head(idd)
pl2 <- ggplot(data = idd, mapping = aes(x = length, y = count)) +
  geom_col() + 
  ggtitle("Indel length distribution") + 
  scale_x_continuous(name = "Indel length") + 
  scale_y_continuous(name = "Total number") + 
  theme_bw()
pl2
ggsave(filename = "indel.png", plot = pl2, width = 6, height = 4)
```
...
```{r pi, eval = F}
pi <- read.table('C:/Users/Miki/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu18.04onWindows_79rhkp1fndgsc/LocalState/rootfs/home/mroman/TAB/ProjectP1/mikel-20191231T125357Z-001/mikel/out.windowed.pi', sep='\t', header=TRUE)
...
```{r answerP1.7}
snpd <- read.table('C:/Users/Miki/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu18.04onWindows_79rhkp1fndgsc/LocalState/rootfs/home/mroman/TAB/ProjectP1/mikel-20191231T125357Z-001/mikel/out.snpden', sep='\t', header=TRUE)
pl4 <- ggplot(data = snpd, mapping = aes(x = BIN_START, y = VARIANTS.KB)) +
  geom_line(color ="red4") +
  labs(title = "SNP density along the chromosome 2 region", x = "Genomic position", y = "Variants/Kbp") +
  theme_bw()
ggplot(data = snpd, mapping = aes(x = BIN_START, y = SNP_COUNT)) +
  geom_line(color ="red4") +
  labs(title = "SNP count along the chromosome 2 region", x = "Genomic position", y = "Count") +
  theme_bw()
pl4
ggsave(filename = "snpden.png", plot = pl4, width = 6, height = 4)
...
```{r answerP1.8}
VEP <- read.table('C:/Users/Miki/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu18.04onWindows_79rhkp1fndgsc/LocalState/rootfs/home/mroman/TAB/ProjectP1/mikel-20191231T125357Z-001/mikel/VEP_test.txt', sep='\t', header=T)
tt <- names(VEP
            )
VEP <- read.table('C:/Users/Miki/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu18.04onWindows_79rhkp1fndgsc/LocalState/rootfs/home/mroman/TAB/ProjectP1/mikel-20191231T125357Z-001/mikel/VEP_telugu.txt', sep='\t', header=F)
colnames(VEP) <- tt

#head(VEP)

pl5 <- ggplot(data = VEP, mapping = aes(x = BIOTYPE)) + 
  geom_bar() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))


pl6 <- ggplot(data = VEP, mapping = aes(x = Consequence)) + 
  geom_bar(width = 1) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggsave(filename = "VEP1.png", plot = pl5, width = 6, height = 4)
ggsave(filename = "VEP2.png", plot = pl6, width = 8, height = 6)
