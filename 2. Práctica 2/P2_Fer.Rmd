...
```{r readSorted}
readDepth <- read.table("C:/Users/Admin/Desktop/sorted.depth.txt.gz", sep='\t', header=FALSE)
...
```{r ST}
st <- read.table("C:/Users/Admin/Desktop/ST.txt", sep='\t', header=FALSE)
...
ggsave(filename = "Substitutions.png", plot = SBS, width = 6, height = 4)
```
...
```{r answerP1.5}
IDD <- read.table("C:/Users/Admin/Desktop/IDD.txt", sep='\t', header=FALSE)
...
```{r answerP1.5A}
LD <- ggplot(data = IDD, mapping = aes(x = length, y = count)) +
...
```{r pi, eval = F}
pi <- read.table("C:/Users/Admin/Desktop/out.windowed.pi", sep='\t', header=TRUE)
...
ND <- ggplot(data = pi, mapping = aes(x = BIN_START, y = PI)) +
  ...
  theme_minimal()
ggsave(filename = "ND_POS.png", plot = ND, width = 6, height = 4)
```
...
```{r answerP1.7}
den <- read.table("C:/Users/Admin/Desktop/out.snpden", sep='\t', header=TRUE)
head(den)
...
SNPd <- ggplot(data = den, mapping = aes(x = BIN_START, y = VARIANTS.KB)) +
  ...
  theme_minimal()
ggsave(filename = "SNPDensity_POS.png", plot = SNPd, width = 10, height = 8)
```
...
```{r answerP1.7A}
Ya está hecho en el anterior directamente
```
...
```{r answerP1.8}
VEP <- read.table("C:/Users/Admin/Desktop/VEP_info.txt", sep='\t', header=TRUE)
head(VEP)
