...
# Using "size" and "alpha" to represent a discrete variable is not recommended because it is not possible to view certain data separately.

# En el caso de size y alpha hay advertencia que anuncia que el uso de ambas para variables discretas no está recomendado por posible pérdida de información.
```
</div>
...
# Using "fill" represents the bar graph filled.

# Con fill el gráfico de barras se vuelve anidado.
```
...
# Data is viewed from horizontal to vertical.

# Los datos pasan a verse de horizontal a vertical.
```
...
# Viridis would look better, due to the color gamut is wider and colorblind people can distinguisg these colores better than the others.

# Se vería mejor viridis, ya que, la gama de colores es más amplia y pueden distinguir los colores mejor las personas con daltonismo.
```
...
# They are used to put a title, a subtitle and a comment.

# Se utilizan para poner título, subtítulo y comentario.
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + theme_classic()

ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + theme_dark()

ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + theme_get()

ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + theme_gray()

ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + theme_grey()

ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + theme_light()

ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + theme_linedraw()

# Classic color maximizes the ink-data ratio because there are no lines in the table.

# El classic maximiza el data-ink ratio porque no hay líneas en la tabla.  
```
...
ggsave(filename = "plot_2.png", plot = p, width = 6, height = 4) # In inches by default
```
...
```{r answer 5.1}
p <- ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length)) + geom_point()
ggsave(filename = "plot_2.svg", plot = p, width = 6, height = 4)
```
...
ggplot(data = economics, mapping = aes(x = date, y = unemploy )) +
  geom_line() + labs(subtitle = "Nivel de desempleo") + theme_bw()

p <- ggplot(data = economics, mapping = aes(x = date, y = unemploy)) + geom_line()
ggsave(filename = "plot_3.png", plot = p, width = 6, height = 4)
```
...
```{r answerP1.1}
ggplot(data = readDepth, mapping = aes(x = position, y = depth, colour = depth)) +
  geom_point()
```
...
```{r answerP1.2}
ggplot(data = readDepth, mapping = aes(x = position, y = depth, colour = depth < 20)) +
  geom_point()
```
...
```{r answerP1.3}
ggplot(data = readDepth, mapping = aes(x = position, y = depth, colour = depth < 20)) + scale_colour_manual( values = c("black", "darkred")) + labs(subtitle = "Read depth along the chromosome 2 region") + theme_light() +
  geom_point()
```
...
```{r answerP1.5}
IDD <- read.table("https://bioinformatica.uab.cat/base/documents/sgbcursos/documents/indel2019_11_5D12_17_36.txt", sep='\t', header=FALSE)
...
ggplot(data = IDD, mapping = aes (x = length, y = count)) +
  ...
  geom_col() + theme_classic() + labs(subtitle = "Read depth along the chromosome 2 region", x = "Indel length", y = "Indel count")
...
```{r answerP1.7}
snpd <- read.table('C:/Users/angel/OneDrive/UNIVERSIDAD/Temas Actuales de la Bioinformática/Ubuntu/ProjectP1/analysis/diversity/out.snpden', sep = '\t', header = TRUE)
ggplot (data = snpd, mapping = aes(x = BIN_START, y = VARIANTS.KB)) +
  geom_line(color = "red4") +
  labs(title = "SNP density along the chromosome 2 region", x = "Genomic position", y = "Variants/kpb") + theme_bw()
ggplot(data = snpd, mapping = aes(x = BIN_START, y = SNP_COUNT)) +
  geom_line(color = "red4") +
  labs(title = "SNP count along the chromosome 2 region", x = "Genomic position", y = "Count") + theme_bw()
```
...
```{r answerP1.8}
read.table("D:/Escritorio/6Kzo7Y47tjIXtNVj.vep.txt")
ggplot
```
